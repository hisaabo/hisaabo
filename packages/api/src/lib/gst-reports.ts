...
export async function generateGSTR1(
  ...
  const isSameState = (partyState: string | null) => {
  ...
  const hsnMap = new Map<string, GSTR1Report["hsn"][0]>();
  ...
  for (const inv of saleInvoices) {
    ...
    const sameState = isSameState(inv.partyState);
    ...
    if (inv.partyGstin) {
      ...
      for (const item of items) {
        const rate = parseFloat(item.taxPercent);
        const existing = b2cSmallMap.get(rate) || { taxRate: rate, taxableValue: 0, cgst: 0, sgst: 0, igst: 0 };
        const itemTaxable = parseFloat(item.totalAmount) - parseFloat(item.taxAmount);
        const itemTax = parseFloat(item.taxAmount);
    ...
    for (const item of items) {
      // Look up HSN from the linked item if available
      const hsn = "0000"; // Default; in production, join with items table
      const existing = hsnMap.get(hsn) || {
        hsn, description: item.description, quantity: 0,
      ...
      existing.taxableValue += itemTaxable;
      existing.totalValue += parseFloat(item.totalAmount);
      if (sameState) {
        existing.cgst += itemTax / 2;
        existing.sgst += itemTax / 2;
      } else {
...
export async function generateGSTR3B(
  ...
  for (const inv of purchaseInvoices) {
    const tax = parseFloat(inv.taxAmount);
    const sameState = biz?.state && inv.partyState &&
