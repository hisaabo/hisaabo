...
When you type the item name on an invoice line item and select the parent item, you need to pick the specific variant before the line item is complete. The variant's sale price fills in automatically.

<ForDesktop>
A **Variant** dropdown appears directly below the item combobox. Select the specific variant from the dropdown list. The list shows each variant's attribute values and its sale price.
</ForDesktop>

<ForMobile>
A **Variant** pill button appears below the item name. Tap it to open a full-screen modal showing all available variants in a scrollable list. Each row displays the variant's attribute values, SKU, and sale price. Tap a variant to select it.
</ForMobile>
...
<ForDesktop>
Click into the item to open its detail slide-over on the right side of the screen. The slide-over shows a breakdown table of every variant, including each variant's attribute values, SKU, current stock quantity, and low-stock status.
</ForDesktop>

<ForMobile>
Tap the item to open its detail screen. Scroll down to see a breakdown of every variant, including each variant's attribute values, SKU, current stock quantity, and low-stock status.
</ForMobile>
