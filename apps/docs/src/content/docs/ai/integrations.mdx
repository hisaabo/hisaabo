...
- Start the Hisaabo MCP server with `--readonly` and a separate write-enabled instance to limit blast radius
...
| GST summary | `hisaabo gst summary --period this-month --json` | Weekly |
| P&L | `hisaabo report pnl --period this-month --json` | Weekly |
...
export async function onInvoiceConfirmed(invoiceId: string) {
  ...
  // Record tracking number back in Hisaabo via CLI
  await execAsync(
    `hisaabo invoice update ${invoiceId} \
      --note "Shiprocket AWB: ${shipment.awb_code}" \
      --tracking-id "${shipment.awb_code}"`
  );
...
mcp_servers:
  - name: hisaabo
    ...
    # Start with --readonly so the agent cannot create or modify records
...
instructions: |
  ...
  For each customer:
  1. Check their outstanding balance via party_outstanding
...
mcp_servers:
  - name: client-sharma-traders
    url: http://localhost:3101/mcp  # MCP server for Client 1
  - name: client-gupta-enterprises
    url: http://localhost:3102/mcp  # MCP server for Client 2
  - name: client-vinod-sons
    url: http://localhost:3103/mcp  # MCP server for Client 3
