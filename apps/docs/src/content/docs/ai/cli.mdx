...
The CLI launches an interactive prompt (powered by Ink) asking for your Hisaabo instance URL, email, and password. On success, it stores your session in `~/.config/hisaabo/config.json`.
...
Session tokens last 30 days. Run `hisaabo login` again when they expire.
...
Clears the stored session from `~/.config/hisaabo/config.json`.
...
Business:      Sharma Traders (business-uuid-here)
```
...
```bash
hisaabo whoami --json
# { "email": "...", "token": "sess_...", "tenantId": "...", "businessId": "...", "businessName": "..." }
```
...
For cron jobs and CI/CD pipelines, environment variables override the config file:

```bash
export HISAABO_API_URL=https://your-hisaabo-instance.com
export HISAABO_TOKEN=sess_VbK2mQ9xP4nR7wA1...
export HISAABO_TENANT_ID=tenant-uuid-here
export HISAABO_BUSINESS_ID=business-uuid-here
```
