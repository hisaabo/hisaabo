...
### Register and profile management
...
#### `hisaabo payment assign-account`
...
#### `hisaabo automated-invoice pause`
...
#### `hisaabo automated-invoice resume`
...
#### `hisaabo automated-invoice run-now`
...
#### `hisaabo automated-invoice delete`
...
| Flag | Description |
|---|---|
| `--tenant` | Tenant slug or UUID to export (required) |
...
| 2 | Rate limit reached (max 2 exports per day) |
| 5 | Server or network error |

:::note
Only tenant owners can export. The CLI does not need a `--business` flag or `HISAABO_BUSINESS_ID` for this command — just authentication and a tenant.
:::
...
| Flag | Description |
|---|---|
| `--tenant` | Target tenant slug or UUID — must be empty (required) |
| `-i, --input` | Input backup file path (required) |
...
:::note
Restore only works on a completely empty tenant (no businesses created). To restore into an existing organization, create a new empty tenant first, restore there, then delete the old one if desired.
:::
...
#### `hisaabo tenant revoke-invitation`
