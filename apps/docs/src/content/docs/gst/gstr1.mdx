...
<ForBusiness>
Your CA will ask for this report every month. Generate it, click **Export CSV**, and email or WhatsApp the file — they'll take it from there. You don't need to understand every section below; skip to [Filing workflow](/gst#filing-workflow-recommended) if you just want the handover checklist.
</ForBusiness>

<ForCA>
:::tip[Before you file]
Two sanity checks before exporting: (1) confirm no invoices are still in `draft` status for the period — drafts **are** counted in the report and are the most common source of post-filing amendments, and (2) spot-check a B2B row against the original invoice PDF for GSTIN and taxable value accuracy.
:::
</ForCA>
