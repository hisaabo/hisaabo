...
<ForCA>
:::tip[CA reading order]
For client reviews, start with the **Party Statement** (full ledger per party), then **Sales Register** and **Purchase Register** for GST reconciliation, then **Tax Summary** to cross-check output/input tax against GSTR-3B before filing. The **Daybook** is your chronological audit trail — export to CSV for working papers.
:::
</ForCA>
