...
| `ENCRYPTION_KEY` | Yes | — | AES-256-GCM key (64-char hex) for field-level encryption. Generate: `node -e "console.log(require('crypto').randomBytes(32).toString('hex'))"` |
| `ENCRYPTION_KEY_PREVIOUS` | No | — | Previous key for rotation — set temporarily, remove after all fields are re-encrypted |
...
When `MULTI_TENANT=true`, Hisaabo provisions a separate PostgreSQL database per tenant (organisation) with a dedicated database user. A separate **control database** stores users, tenants, sessions, and tenant members. Tenant database credentials are encrypted at rest using `ENCRYPTION_KEY`.
