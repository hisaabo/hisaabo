...
export default function ProformaInvoicesScreen() {
  ...
  const { data, isLoading, isError, refetch, isRefetching } =
    trpc.proforma.list.useQuery(queryInput);

  const deleteMutation = trpc.proforma.delete.useMutation({
    onSuccess: () => { utils.proforma.list.invalidate(); haptic.success(); },
    onError: (err) => { haptic.error(); Alert.alert("Error", err.message); },
  });

  const updateStatusMutation = trpc.proforma.updateStatus.useMutation({
    onSuccess: () => { utils.proforma.list.invalidate(); haptic.success(); },
    onError: (err) => { haptic.error(); Alert.alert("Error", err.message); },
  });
  ...
  const handleConvert = useCallback(
    (id: string, num: string) => {
      Alert.alert(
        "Convert to Invoice",
        `Convert proforma ${num} into a sales invoice?`,
        [
          { text: "Cancel", style: "cancel" },
          {
            text: "Convert",
  ...
  const ListHeader = (
    <View style={styles.listHeader}>
      <View style={styles.headerBar}>
        ...
        <Text style={styles.screenTitle}>Proforma Invoices</Text>
        <View style={{ width: 40 }} />
      </View>
      ...
      {!isLoading && (
        <Text style={styles.countText}>
          {total} {total === 1 ? "proforma invoice" : "proforma invoices"}
        </Text>
      )}
    </View>
  );
  ...
  const ListEmpty = isError ? (
    <QueryError message="Failed to load proforma invoices" onRetry={refetch} />
  ) : isLoading ? null : (
    <EmptyState
      icon="document-text-outline"
      title="No proforma invoices found"
      description={search ? "Try a different search term" : "Create your first proforma invoice"}
    />
  );
