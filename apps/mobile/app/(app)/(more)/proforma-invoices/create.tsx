...
function in30daysDate() { const d = new Date(); d.setDate(d.getDate() + 30); return d; }
    ...
    <Modal visible={visible} animationType="slide" transparent presentationStyle="overFullScreen">
      <View style={ms.overlay}>
        <View style={ms.sheet}>
          ...
          <FlatList data={filtered} keyExtractor={(item) => item.id} contentContainerStyle={ms.listContent}
            renderItem={({ item }) => (
...
export default function ProformaCreateScreen() {
  ...
  const handleCreate = useCallback(() => {
    ...
    createMutation.mutate({
      partyId: selectedParty.id, type: "sale", documentType: "proforma",
        ...
        <ScrollView style={s.scroll} contentContainerStyle={s.scrollContent} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
          ...
          <View style={s.datesRow}>
            <View style={s.dateCard}>
              <DatePickerField label="Proforma Date" value={invoiceDate} onChange={setInvoiceDate} />
            </View>
            <View style={s.dateCard}>
              <DatePickerField label="Valid Until" value={dueDate} onChange={setDueDate} minimumDate={invoiceDate} />
            </View>
          </View>
          ...
          <TextInput style={s.notesInput} value={notes} onChangeText={setNotes} placeholder="Terms, conditions, validity, etc." placeholderTextColor={colors.textMuted} multiline numberOfLines={3} textAlignVertical="top" />
          ...
          <TouchableOpacity style={[s.createBtn, createMutation.isPending && s.createBtnDisabled]} onPress={handleCreate} activeOpacity={0.85} disabled={createMutation.isPending}>
            {createMutation.isPending ? <ActivityIndicator color={colors.textPrimary} size="small" /> : (
              <>
                ...
                <Text style={s.createBtnText}>Create Proforma Invoice</Text>
              </>
            )}
          </TouchableOpacity>
        </View>
...
const s = StyleSheet.create({
  ...
  dateCard: { flex: 1, backgroundColor: colors.surface, borderRadius: 14, borderWidth: 1, borderColor: colors.border, padding: 14 },
