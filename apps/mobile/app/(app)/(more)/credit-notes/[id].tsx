...
export default function CreditNoteDetailScreen() {
  ...
  const deleteDoc = trpc.creditNote.delete.useMutation({
    onSuccess: () => {
      haptic.success();
      utils.creditNote.list.invalidate();
  ...
  const handleMarkSent = () => {
    if (!doc) return;
    haptic.medium();
    Alert.alert("Mark as Sent", "Mark this credit note as sent?", [
  ...
  if (!doc) {
    return (
      <SafeAreaView style={styles.container} edges={["top"]}>
        ...
        <QueryError message="Credit note not found" onRetry={() => refetch()} />
      </SafeAreaView>
    );
  }
        ...
        {doc.status === "draft" && (
          <View style={[styles.actionGroup, styles.dangerGroup]}>
            ...
            >
              ...
              <Text style={[styles.actionBtnText, { color: colors.danger }]}>Delete Credit Note</Text>
            </TouchableOpacity>
          </View>
        )}
      </ScrollView>
