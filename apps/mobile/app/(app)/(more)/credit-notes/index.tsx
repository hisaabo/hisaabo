...
export default function CreditNotesScreen() {
  ...
  const { data, isLoading, isError, refetch, isRefetching } =
    trpc.creditNote.list.useQuery(queryInput);
  ...
  const ListHeader = (
    <View style={styles.listHeader}>
      <View style={styles.headerBar}>
        ...
        <Text style={styles.screenTitle}>Credit Notes</Text>
        <View style={{ width: 40 }} />
      </View>
  ...
  const ListEmpty = isError ? (
    <QueryError message="Failed to load credit notes" onRetry={refetch} />
  ) : isLoading ? null : (
    <EmptyState
      icon="return-down-back-outline"
      title="No credit notes found"
      description={search ? "Try a different search term" : "Create your first credit note"}
    />
  );
  ...
  return (
    <SafeAreaView style={styles.container} edges={["top"]}>
      ...
      <FAB onPress={() => router.push("/(more)/credit-notes/create" as never)} />
    </SafeAreaView>
  );
}
